...
6. **Type pairing.** UI / body / transcript → **Onest**. Timestamps / IDs / code → **IBM Plex Mono**. Serif was removed in B18; Hanken Grotesk was replaced by Onest in TD-47 (it has no base Cyrillic, so the whole Russian and Kazakh UI fell back to the system font) — do not reintroduce either. Sizes come from the `--t-11..28` scale.
