/* eslint-disable */
// WOTOLD · screens A — Inbox (database table) + Call (doc + rail + assistant)
...
function CallView({ call, engine, setEngine, onBack }) {
  ...
  const ask = (q) => {
    const match = WK_ASK.find((x) => x.q === q) || WK_ASK[0];
    setThread((t) => [...t, { me: true, text: q }, { me: false, text: match.a }]);
    setDraft('');
  };

  const tabs = [{ value: 'transcript', label: 'Транскрипт', icon: 'doc' }, { value: 'recap', label: 'Рекап', icon: 'sparkle' }];
  if (ready) tabs.push({ value: 'assistant', label: 'Ассистент', icon: 'command' });
        ...
        <div className="content doc-wrap">
          <div className="doc-scroll scroll">
            <div className="doc" key={call.id} style={{ paddingBottom: ready && tab !== 'assistant' ? 104 : undefined }}>
              ...
              {call.status === 'error' ? <ErrorCard /> : <>
                ...
                {tab === 'assistant' && ready && <Assistant thread={thread} onAsk={ask} />}
              </>}
            </div>
          </div>
          ...
          {ready && tab === 'assistant' && (
            <div className="composer-dock">
              <form className="composer composer-ask" onSubmit={(e) => { e.preventDefault(); if (draft.trim()) ask(draft.trim()); }}>
...
function Recap({ actions, setActions }) {
  ...
  const copy = () => { try { navigator.clipboard.writeText(md); } catch (e) {} setCopied(true); setTimeout(() => setCopied(false), 1400); };
  return (
    <div style={{ marginTop: 18 }}>
...
function Assistant({ thread, onAsk }) {
  return (
    <div style={{ marginTop: 18, paddingBottom: 80 }}>
      {thread.length === 0 && (
        <div style={{ color: 'var(--text-3)', fontSize: 13.5, marginBottom: 14 }}>
          Спросите что угодно об этом звонке — ответ строится по расшифровке.
        </div>
      )}
      <div className="ask-thread">
        {thread.map((m, i) => (
          <div className="ask-row" data-me={m.me} key={i}><div className="ask-bubble">{m.text}</div></div>
        ))}
      </div>
      <div className="ask-suggest" style={{ marginTop: thread.length ? 16 : 0 }}>
        {WK_ASK.map((s) => <Chip key={s.q} tone="line" icon="arrowRight" onClick={() => onAsk(s.q)}>{s.q}</Chip>)}
      </div>
    </div>
  );
}
