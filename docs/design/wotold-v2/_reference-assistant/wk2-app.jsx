/* eslint-disable */
// WOTOLD · app shell (форк для страницы «Ассистент») — + раздел «Ассистент» в навигации и роутинге
...
function MiniRail({ view, recording, paused, theme, onExpand, onNav, onRecord, onPause, onSearch, onToggleTheme, onResizeStart }) {
  return (
    <aside className="minirail">
      ...
      <IconBtn icon="command" label="Найти или спросить" tip="Найти или спросить ⌘K" tipSide="right" onClick={onSearch} />
      <div className="minirail-sep" />
      ...
      <IconBtn icon="chat" label="Ассистент" tip="Ассистент" tipSide="right" active={view === 'assistant'} onClick={() => onNav('assistant')} />
  ...
  return (
    <aside className="rail">
      ...
      <div style={{ padding: '0 10px' }}>
        <button className="input ai-field ai-field--panel" onClick={onSearch} style={{ cursor: 'pointer', height: 30, color: 'var(--text-3)' }}>
          <Icon name="sparkle" size={15} className="iico" />
          <span style={{ flex: 1, textAlign: 'left', fontSize: 13, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>Найти или спросить</span>
          <Kbd>⌘K</Kbd>
        </button>
      </div>
      ...
      <nav className="scroll" style={{ flex: 1, minHeight: 0, padding: '10px' }}>
        ...
        <NavItem icon="chat" label="Ассистент" active={view === 'assistant'} onClick={() => onNav('assistant')} />
...
// ── Command palette ──
function Palette({ onClose, onNav, onOpenCall, onRecord, onAsk }) {
  const [q, setQ] = uS('');
  ...
  const actions = [
    ...
    { id: 'assist', icon: 'chat', label: 'Ассистент — поиск по звонкам', run: () => onNav('assistant') },
  ...
  const askFallback = ql && fA.length === 0 && fC.length === 0 ? { id: 'ask', kind: 'q', label: q.trim() } : null;
  const flat = [...fA.map((a) => ({ ...a, kind: 'a' })), ...fC.map((c) => ({ id: c.id, label: c.title, kind: 'c' })), ...(askFallback ? [askFallback] : [])];
  const exec = (it) => { it.kind === 'a' ? it.run() : it.kind === 'q' ? onAsk(it.label) : onOpenCall(it.id); onClose(); };
  const onKey = (e) => {
  ...
  return (
    <div className="overlay fade" onMouseDown={onClose}>
      <div className="palette fade-up ai-field" onMouseDown={(e) => e.stopPropagation()}>
        <div className="palette-input">
          <Icon name="search" size={18} style={{ color: 'var(--text-faint)' }} />
          <input ref={ref} placeholder="Найти звонок или спросить ассистента…" value={q} onChange={(e) => setQ(e.target.value)} onKeyDown={onKey} />
          <Kbd>esc</Kbd>
        </div>
        <div className="palette-list scroll">
          ...
          {askFallback && <>
            <MenuLabel>Ничего не найдено · Ассистент</MenuLabel>
            <MenuItem icon="chat" active={sel === flat.length - 1} end={<Kbd>↵</Kbd>} onClick={() => exec(askFallback)}>
              <div style={{ fontWeight: 550 }}>Спросить ассистента</div>
              <div className="u-faint" style={{ fontSize: 11.5 }}>«{askFallback.label}» — поиск по {AS_STATS.calls} звонкам</div>
            </MenuItem>
          </>}
...
function App() {
  ...
  // эскалация из чата звонка: новый чат в общем ассистенте с тем же запросом
  const askGlobal = (q) => { WK_AS_NEWCHAT(q); setActiveCallId(null); setView('assistant'); };
  ...
  return (
    <div className="app" data-collapsed={collapsed} style={{ ['--rail-w']: railW + 'px' }}>
      ...
      <main className="main">
        ...
        {view === 'assistant' && <AssistantView onOpenCall={openCall} />}
        ...
        {view === 'call' && activeCall && <CallView key={activeCall.id} call={activeCall} engine={engine} setEngine={setEngine} onBack={() => nav('inbox')} onOpenCall={openCall} onAskGlobal={askGlobal} />}
      ...
      {paletteOpen && <Palette onClose={() => setPaletteOpen(false)} onNav={nav} onOpenCall={openCall} onRecord={toggleRec} onAsk={askGlobal} />}
    </div>
  );
}
