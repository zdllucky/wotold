...
- [ ] **B3.7c** Model deployment — runtime download `wespeaker_en_voxceleb_resnet34_LM.onnx` (25MB) с https://github.com/k2-fsa/sherpa-onnx/releases/tag/speaker-recongition-models в `$APP_DATA/models/embedder.onnx` + SHA256 check + UI splash «Скачиваем модель распознавания голоса (25MB)...» на первой записи. Integration test против reference embedding для зашитого WAV (sherpa-onnx test fixtures fangjun-sr-1.wav) ≤ ε. Decision pending: bundle модель в .dmg (>100MB → Apple notarization tier R6) vs runtime download (offline-first под угрозой если первая запись без сети).
