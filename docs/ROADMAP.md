...
## B32 · Чистка настроек по итогам просмотра экранов

- [x] **B32.1** (M) Потолок числа спикеров `3 → 10`. Тройка была продуктовым решением (P14.3), а не ограничением движка, и применялась после инференса: индексы сверх неё уходили в «неизвестного», а `reassign_unknown_to_neighbors` приклеивал их к ближайшему по времени спикеру — на звонке впятером четвёртый и пятый молча приписывались другим людям. Настройка «число собеседников» удалена целиком вместе с механикой ручного переопределения (поле, клэмпы, аксессор, параметр через восемь файлов), мёртвым `apply_speaker_cap` и осиротевшими ключами локалей.
- [x] **B32.2** (S) Шапка «О ПРИЛОЖЕНИИ» убрана: дублировала хлебную крошку и `aria-label`, а остальные разделы настроек шапок не имели.
- [x] **B32.3** (S) Карточка «Модуль распознавания голоса» убрана из «Спикеров»: модуль обязательный и ставится сам, а рядом с баннером готовности она была вторым индикатором прогресса на тот же файл. Предупреждение о сборке без `voice-onnx` осталось — другого места, где это видно, нет.
- [x] **B32.4** (M) Прошлое поколение дизайна удалено из репозитория: каталог `docs/design/atelier-v2/` (14 файлов), осиротевший UX-бриф M12, мёртвое правило `.dot--signal`, два десятка комментариев-отсылок. Живой код это задело в одном месте — механике акцентов (`Accent`, `data-accent`, `ui.accent`), пережившей удаление пикера в B18.5 и ни на что не влиявшей. История (`ROADMAP_ARCHIVE.md`, `docs/audits/`) не тронута.
