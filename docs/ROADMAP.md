...
- **#42 X1 Tauri minisign keygen** — `[владелец]` — `pnpm tauri signer generate`; public → `tauri.conf.json`, private+password → GH Secret + офлайн-бэкап (M11.1/M11.9). Без него апдейтер скачивает обновление, но не может проверить подпись и не ставит его. Пайплайн релиза, политика обновлений и Homebrew-каск готовы и ждут только ключа.
