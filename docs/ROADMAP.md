...
**Deferred до получения ONNX model bytes (B3.7):**
...
- [~] **B3.7a scaffold** — Cargo feature `voice-onnx` + `ort = "=2.0.0-rc.10"` (load-dynamic — `download-binaries` сломан в ort-sys 2.0.0-rc.12 из-за ureq 3.3 incompat) + `ndarray = "0.16"`. Новый модуль `src/embeddings_onnx.rs` под `#[cfg(feature = "voice-onnx")]` со скаффолдом `OnnxEmbedder`: `load(path)` грузит `ort::Session` + валидирует 1 input + 1 output, `extract()` возвращает `Err("fbank preprocessing not implemented yet (B3.7b)")`. `try_load_onnx_embedder` диспетчер в `embeddings.rs` под фичей вызывает новый модуль. Default build (без фичи) — деп tree без ort/ndarray. cargo check / clippy / test зелёные на обоих режимах. Подтверждается comp-time assertion'ом `EMBEDDING_DIM == 256` (WeSpeaker/ECAPA gates).
- [ ] **B3.7b** Kaldi mel-fbank preprocessing — 80-dim mel-filterbank, 25ms window, 10ms hop @ 16kHz, CMN per-utterance. Unit-test против reference `torchaudio.compliance.kaldi.fbank()` на зафиксированном WAV (baked в test fixtures). Без exact replication — embeddings garbage.
- [ ] **B3.7c** ONNX inference + WeSpeaker model + integration test — `extract()` собирает batch (1, T, 80) fbank tensor, вызывает `session.run()`, L2-normalize, возвращает 256-dim. Integration test против reference Python embedding ≤ ε. Bundle vs runtime download + ONNX Runtime native lib bundling — отдельный design call (R6 notarization).
