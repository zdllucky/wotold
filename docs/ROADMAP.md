...
- [ ] **B3.7** Real OnnxEmbedder — `voice-onnx` feature: `ort` + `ndarray` + Kaldi-style 80-dim mel-fbank (25ms/10ms, CMN) + WeSpeaker ResNet34/ECAPA-TDNN ONNX в `$APP_DATA/models/embedder.onnx` + integration test (известный WAV → reference Python WeSpeaker embedding ≤ ε). Bundle vs runtime download — отдельный design call (notarization size policy R6).
