...
process.stdin.on('end', () => {
  ...
  // [TD-49] Словари переводов из лимита исключены — решение владельца.
  // Правило 8 писалось про модули: «много маленьких файлов» лечит когезию
  // кода. Локаль — плоская таблица строк, у которой когезия по определению
  // одна; резать её по неймспейсам значит платить тремя правками в трёх
  // файлах за каждую новую строку и получать расхождения между локалями.
  // Гейт, который блокирует то, что чинить не собираются, просто врёт.
  const isLocaleDict = /(^|\/)i18n\/[a-z]{2}(-[A-Z]{2})?\.ts$/.test(path);

  const { before, after } = resultingLines(path, input?.tool_input);
  if (!isLocaleDict && after !== null && after > 800 && (before === null || after > before)) {
