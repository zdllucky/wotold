...
| R14 | Базовые модули обязательны, per-module тумблеров нет: не хватает любого — обработка стоит, звонки паркуются и поднимаются сами после докачки (B32) |
| R15 | Авто-**стоп** по тишине + подрезка тихого хвоста — делаем (R3 про авто-старт, стоп им не покрыт). Рамки: сигнал только RMS `max(mic, system)`, пауза тишиной не считается, подсказка фиксирована 15 мин, стоп настраиваемый 30/60/120/`never` (default 30), рез только у авто-остановленной записи, факт реза виден degraded-чипом. Секция T роадмапа |
...
### Разрешения macOS и Info.plist

Микрофон и системный звук пишет сайдкар `wotold-audio`, он же дёргает TCC. Но TCC атрибутирует запрос **ответственному процессу** — приложению, — поэтому usage-описания обязаны быть в `Info.plist` основного бандла, а не только сайдкара. Без `NSMicrophoneUsageDescription` там macOS убивает сайдкар SIGABRT'ом (`namespace: TCC`) на первом же `AVCaptureDevice.requestAccess`, и наружу приезжает лишь «sidecar exited without permissions event».

Ключи лежат в [`apps/desktop/src-tauri/Info.plist`](apps/desktop/src-tauri/Info.plist). Отдельного файла для dev не нужно: `tauri_build::build()` подхватывает этот plist рядом с `tauri.conf.json` сам — в релизе домешивает в Info.plist бандла, в debug вшивает в бинарь секцией `__TEXT,__info_plist` (у `tauri dev` бандла нет вовсе). Своего `-sectcreate` в `build.rs` быть не должно: он положит в ту же секцию второй plist-документ. Проверка — `otool -s __TEXT __info_plist target/debug/wotold-desktop`.

Второй источник «выдано в настройках, а приложение пишет отказано» — ad-hoc подпись (R6): cdhash меняется на каждой сборке, грант отваливается после обновления. Лечится кнопкой «Сбросить доступ» в Настройках → Разрешения (`tccutil reset`), см. TECH_DEBT.
