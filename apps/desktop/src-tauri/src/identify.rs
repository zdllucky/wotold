...
//!
//! Принимает `Box<dyn Embedder>` — production wire с реальным OnnxEmbedder
//! сделаем когда модель будет (drop-in замена). Сейчас вызов отсутствует в
//! `pipeline::run` — модуль готов к использованию из #26 (UI confirm flow).
//!
//! Шаги:
