...
// Тумблера «Семантический поиск» больше нет: файлы эмбеддера входят в
// обязательный набор движка (см. `local_engine::readiness::base_model_ids`), и
// качает их `provisioning::ensure_required` — по явному нажатию пользователя, а
// не фоном на старте. Отсутствие файлов означает «модули ещё не докачаны», и
// об этом говорит баннер, а не молчаливая деградация поиска до подстрочного.
