...
describe('ProgressRail', () => {
  test('determinate sets aria-valuenow + width', () => {
    ...
    const fill = document.querySelector('.rail-fill') as HTMLElement | null;
  ...
  test('clamps values <0 and >100', () => {
    ...
    expect(
      (document.querySelector('.rail-fill') as HTMLElement | null)?.style.width,
    ).toBe('0%');
  ...
  test('indeterminate adds modifier + uses aria-valuetext', () => {
    ...
    const rail = document.querySelector('.rail');
    expect(rail?.classList.contains('rail--indeterminate')).toBe(true);
