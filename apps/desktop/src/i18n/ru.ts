...
const ruInternal = {
  ...
  errors: {
    ...
    permissionsProbe: {
      human: 'Не удалось проверить разрешения — компонент записи звука не ответил.',
      hint: 'Перезапусти Wotold. Если повторится — переустанови приложение и сообщи нам.',
    },
    permissionsProbeTimeout: {
      human: 'Запрос разрешения остался без ответа.',
      hint: 'Системный диалог мог остаться незамеченным за другим окном. Нажми «Запросить» ещё раз.',
    },
    permissionReset: {
      human: 'Не удалось сбросить доступ.',
      hint: 'Открой Системные настройки → Конфиденциальность и безопасность и сними галочку с Wotold вручную, затем запроси доступ заново.',
    },
    micPermission: {
  ...
  permissions: {
    ...
    staleHint:
      'Если ты уже разрешал этот доступ — macOS мог его потерять: он привязан к подписи приложения и не переживает обновление Wotold. Галочка в Системных настройках при этом остаётся на месте.',
    reset: 'Сбросить доступ',
    resetTitle: 'Сбросить доступ и запросить заново',
    resetBody:
      'macOS забудет прошлый ответ на этот запрос и снова покажет системный диалог. Ничего кроме этого разрешения не затрагивается.',
    resetConfirm: 'Сбросить и запросить',
    request: 'Запросить',
  ...
  voiceModel: {
    featureOff:
      '⚠ Эта сборка собрана без voice-onnx: голоса не сравниваются, и привязка спикеров к контактам работать не будет. В обычной сборке фича включена.',
  },
  ...
  recording: {
    ...
    // [T7/R15] SilencePrompt — тишина в идущей записи.
